...
describe('discovery fallback behavior', () => {
  it('uses advertisement.id when BLE address is an empty string', () => {
    ...
    ;(switchbot as any).handleBLEDiscovery({
  ...
  it('uses advertisement.id when BLE address is missing', () => {
    const switchbot = new SwitchBot({
      enableBLE: false,
      token: '',
      secret: '',
  ...
  it('extracts MAC from manufacturer data when address is empty', () => {
    const switchbot = new SwitchBot({
      enableBLE: false,
      token: '',
      secret: '',
  ...
  it('supports legacy service UUID (000d)', () => {
    const switchbot = new SwitchBot({
      enableBLE: false,
      token: '',
      secret: '',
  ...
  it('stores and retrieves devices by BLE ID when MAC is unavailable', () => {
    const switchbot = new SwitchBot({
      enableBLE: false,
      token: '',
      secret: '',
