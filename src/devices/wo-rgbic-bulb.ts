 ...
 */
export class WoRGBICBulb extends WoBulb implements RGBICBulbCommands {
  private static readonly RGBIC_EFFECTS: Record<string, number> = {
   ...
   */
  async setSegmentColor(segmentId: number, red: number, green: number, blue: number): Promise<boolean> {
   ...
   */
  async setSegmentEffect(segmentId: number, effectName: string, speed = 50): Promise<boolean> {
    ...
    const result = await this.sendCommand(
      bleCommand,
      'setSegmentEffect',
      `seg${segId}:${effectName}:${effectSpeed}`,
    )
    return result.success
  }
