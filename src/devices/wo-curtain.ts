 ...
 */
export class WoCurtain extends SwitchBotDevice implements CurtainCommands {
  /**
   * Open curtain (position 0%)
   */
  async open(speed = 255): Promise<boolean> {
    const clampedSpeed = clamp(speed, 1, 255)
    const result = await this.sendCommand(
      [...DEVICE_COMMANDS.CURTAIN.POSITION, clampedSpeed, 0],
      'setPosition',
      `0,${clampedSpeed.toString(16).padStart(2, '0')},0`,
    )
   ...
   */
  async close(speed = 255): Promise<boolean> {
    const clampedSpeed = clamp(speed, 1, 255)
    const result = await this.sendCommand(
      [...DEVICE_COMMANDS.CURTAIN.POSITION, clampedSpeed, 100],
      'setPosition',
      `0,${clampedSpeed.toString(16).padStart(2, '0')},100`,
    )
    return result.success
  }
   ...
   */
  async setPosition(position: number, speed = 255): Promise<boolean> {
    ...
    const result = await this.sendCommand(
      bleCommand,
      'setPosition',
      `0,${clampedSpeed.toString(16).padStart(2, '0')},${clampedPosition}`,
    )
    return result.success
  }
   ...
   */
  async getExtendedInfo(): Promise<CurtainExtendedInfo> {
    if (!this.hasBLE()) {
