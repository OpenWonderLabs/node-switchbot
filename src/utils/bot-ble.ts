 ...
 */
function calculateCRC32(input: string): number {
  let crc = 0xFFFFFFFF
...
export const BOT_BLE_ACTIONS = {
  PRESS: 0x00 as BotBleAction,
  TURN_ON: 0x01 as BotBleAction,
  TURN_OFF: 0x02 as BotBleAction,
}
...
export function validateBotPassword(password: string): void {
  if (!PASSWORD_REGEX.test(password)) {
    throw new Error(
      'Invalid Bot password. Password must be exactly 4 alphanumeric characters (case-sensitive).',
    )
  }
}
 ...
 */
export function buildBotBleCommand(
  action: BotBleAction,
  password?: string,
): Buffer {
  // Validate action
  ...
  if (!validActions.includes(action)) {
    throw new Error(
      `Invalid Bot BLE action: 0x${action.toString(16)}. Expected 0x00, 0x01, or 0x02.`,
    )
  }
 ...
 */
export function parseBotBleResponse(response: Buffer): boolean {
