...
import { WoSmartLockPro } from './wosmartlockpro';
...
describe('WoSmartLockPro', () => {
  ...
  describe('unlock', () => {
    it('should unlock the smart lock', async () => {
      ...
      expect(operateLockProStub.calledOnceWith(WoSmartLockPro.COMMAND_UNLOCK)).to.be.true;
    });
  });
  ...
  describe('unlockNoUnlatch', () => {
    it('should unlock the smart lock without unlatching', async () => {
      ...
      expect(result).to.equal(WoSmartLockPro.Result.SUCCESS);
      expect(operateLockProStub.calledOnceWith(WoSmartLockPro.COMMAND_UNLOCK_NO_UNLATCH)).to.be.true;
    });
  });
  ...
  describe('lock', () => {
    it('should lock the smart lock', async () => {
      ...
      expect(result).to.equal(WoSmartLockPro.Result.SUCCESS);
      expect(operateLockProStub.calledOnceWith(WoSmartLockPro.COMMAND_LOCK)).to.be.true;
    });
  });
  ...
  describe('info', () => {
    it('should get the lock info', async () => {
      ...
      expect(operateLockProStub.calledOnceWith(WoSmartLockPro.COMMAND_LOCK_INFO)).to.be.true;
    });
  });
  ...
  describe('getIv', () => {
    it('should get the IV', async () => {
      ...
      expect(operateLockProStub.calledOnceWith(WoSmartLockPro.COMMAND_GET_CK_IV + lock._key_id, false)).to.be.true;
    });
  });
  ...
  describe('operateLockPro', () => {
    it('should operate the lock with encryption', async () => {
      ...
      expect(result).to.be.instanceOf(Buffer);
      expect(encryptedCommandStub.calledOnceWith(key)).to.be.true;
    });
